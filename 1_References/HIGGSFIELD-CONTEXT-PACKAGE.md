...
description: Operational guide for provider/reference prompt assembly. Defines exactly which reference images to attach or describe and what visual context to include in the prompt for each visual style. Load during Stage 7 assembly and Stage 8 generation alongside CREATIVE-DIRECTION.md.
---

# Higgsfield Context Package — Visual Reference Intelligence

> **When to load:** Stage 7 assembly and Stage 8 generation, before firing any prompt to Codex ImageGen or Higgsfield fallback.
...
**Repeatable 3-reference recipe.** Within the hard 3-ref cap, every call is **[1] colour swatch (always) + [2] the one style-defining ref + [3] visible logo (only if logo gate = yes)**. If logo = no, slot 3 frees for the next-priority style ref; anything that doesn't fit goes into the prompt text.
...
| Infographic / icon | `09_Icon-System/02_icon-system-overview.png` by default; use `4_Infographic Icon References/soft-dimensional-glass-icons-blue.png` when soft dimensional polish is selected or needed |
...
**Archetype 8 — No-text balanced hero.** When visible text is `None`, or the user asks for no headline/body/subtext, the layout ref is `NT-1` and the archetype is `no-text-balanced-hero`. This applies across all visual styles. Prompt no visible text, no subtext, no labels, and no dormant text zone. Balance the centred or near-centred hero/form/icon system with full-canvas pattern/atmosphere, comparable left/right breathing room, and no edge-crowded cutout. Style-specific interpretation: photo = centred photo cutout; illustration = centred illustrated cutout; infographic = centred icon/module hero with no labels unless requested; abstract = centred dot-form hero.

**1 — Colour anchor (always attach).** Attach `1_Brand Guidelines/02_Color-System/brand-blue-4736FE-swatch.png` on dark calls — caption *"match the whole empty background canvas to Cars24 Brand Blue #4736FE; do not render the swatch."* Attach `1_Brand Guidelines/02_Color-System/brand-blue-lavender-EBE9FF-swatch.png` on light calls — caption *"match the light background to exact pale lavender #EBE9FF derived from Cars24 brand blue; do not render the swatch."* Image models can average all refs' colours, so without this the brand blue can drift to navy/indigo (Δ70–147). Always describe the hue with **exact hex + plain-language guardrails**: dark = *"Cars24 Brand Blue #4736FE as the dominant full-bleed canvas; add a subtle same-hue vertical or ambient gradient for premium depth, with an optional restrained radial glow around the hero/pattern zone. The gradient may drift slightly lighter or darker around #4736FE but must still read as bright Cars24 brand blue — NOT navy, NOT indigo, NOT black, NOT midnight blue, NOT dark violet, NOT dimmed AI-tech dark mode. Do not render the hex code as text."* Light = *"exact pale lavender #EBE9FF background derived from Cars24 Brand Blue; headline and short punch/tagline in #4736FE, descriptive body in #161616; do not render the hex codes as text."* Keep gradients controlled and same-hue; avoid "dark background", "deep blue", "midnight", "void", "black", "dramatic shadows", and "high-contrast dark field" for the canvas.

**2 — Visible logo context (attach if logo gate = yes).** On dark/brand-purple attach `1_Brand Guidelines/01_Brand-Identity-&-Logo/Logos/Logo - White-on-blue.png`; on light attach `Logo - Blue-on-white.png`; on high-contrast/print attach `Logo - Black.png`. The logo must be rendered inside the generated composite from this reference. Do not create a post-process/local logo overlay. Size it consistently with reference creatives (about 8–10% canvas width for slide branding), place it in clean negative space, preserve clear space, and render it with no box/tile.
...
Close-up bust portrait of a South Asian woman on a light sky-blue background (ignore this background unless it is explicitly chosen as layout context; the generated subject should render as a cutout-style hero inside the themed Cars24 composite). Navy-black hair, warm caramel skin. Clean aspirational face — almond eyes, defined brows, subtle confident smile. The character fills ~60% of the frame. Use when the scene is a portrait or upper-body composition.
...
```
Style: modern sleek flat editorial illustration in Cars24 brand visual language. Electric brand blue dominant colour — approximately 60% of the subject. Warm caramel South Asian characters with deep navy-black hair. Clean flat colour shapes with minimal shading (1–2 tonal steps per zone only). No gradients, no photorealistic textures, no environmental backgrounds. Clean silhouette edges. Render the subject as a clean-edged cutout-style hero with no scene of its own — no sky, no cityscape, no background fill, no box — fully contained inside the themed brand canvas in this same generation, with full head, hands, car/icons, and key objects visible. No text, numbers, or labels drawn into the subject art. Aspirational, confident, premium mood.
...
LOGO (if requested): render the Cars24 logo from the attached visible logo reference at [bottom-left / centre-bottom for centre layouts], inside clean negative space, approximately 8–10% of canvas width, with clear space. Reproduce the rounded-square icon mark + "Cars24" wordmark faithfully in the correct theme colourway — white on dark, vivid Brand Blue #4736FE on light (never greyed or muted). No box/tile, no placeholder, no decorative effect, no later local overlay.
```
...
- Full creative prompts may use exact hex locks for canvas, text, logo, and icon colours, always with "do not render the hex code as text." Inside the illustration subject art itself, use descriptive colour names ("electric brand blue", "warm caramel", "deep navy-black").
...
LOGO (if requested): render the Cars24 logo from the attached visible logo reference inside clean negative space, approximately 8-10% of canvas width, with clear space. Reproduce the rounded-square icon mark + "Cars24" wordmark faithfully in the correct theme colourway — white on dark, vivid Brand Blue #4736FE on light. No box/tile, no placeholder, no local overlay.
```
...
LOGO (if requested): reproduce the Cars24 logo from the attached visible logo reference — icon mark + "Cars24" wordmark — exactly, in white on dark, vivid Brand Blue #4736FE on light (never greyed or muted), bottom-left, no box/tile; ignore the blue tile of the logo reference.
```
...
Use for: product features, marketing callouts, landing pages. Visual rules: moderate 3D realism (not photoreal — like high-quality iOS app icons). Consistent blue-based colour system. Crisp highlights from top-left, controlled subtle shadow. Simple subjects — 1–2 elements per icon. White or transparent background.
...
**`soft-dimensional-glass-icons-blue.png`** — Soft dimensional glass inspiration
Use for: premium marketing infographic slides, process flows, feature symbols, and icon-led explanations that should feel richer than flat UI icons but lighter than full 3D product icons. The generated icon must still be semantically correct for the slide subject. Visual rules to copy: filled rounded geometric shapes, translucent pale-blue/frosted-glass front layers over saturated Cars24 brand-blue base layers, soft internal blur at overlaps, gentle top-left highlight, minimal details, no text inside icons. Do not copy app-tile layout or generic glassmorphic UI framing.
...
**Semantic-first icon rule** — choose the icon concept that best explains the slide before choosing finish. Prefer soft dimensional glass polish for premium/process icons when it improves output, but use 3D or flat filled when those styles communicate the subject better or when glass becomes generic, broken, or unclear. The soft-dimensional-glass images may teach softness, highlight discipline, rounded filled forms, and gentle depth. They must not replace semantic accuracy, create glass UI tiles, create one large glass slab, or push outputs toward generic app-icon sheets.

### Reference attachment protocol

| Task | Always attach (slot [2]) | Also attach |
|---|---|---|
| Any infographic / icon brief (default) | `02_icon-system-overview.png` | choose semantic icon concepts before finish |
| Premium marketing / process icons with soft polish | `soft-dimensional-glass-icons-blue.png` when slot [2] is available | fall back to `02_icon-system-overview.png` + prompt text if logo consumes the slot |
| Premium marketing / feature callout icons where depth helps meaning | `02_icon-system-overview.png` | `03_3d-icon-generator.png` when slot free |
...
| Soft dimensional glass inspiration | `soft-dimensional-glass-icons-blue.png` | only when it supports semantic clarity |
...
**Plus, every call (see Universal generation defaults):** slot [2] is the strongest style/semantic ref available after the mandatory colour swatch and any visible logo reference; always add the `#4736FE` colour swatch as slot [1], and the visible logo context (`Logo - White-on-blue.png` dark / `Logo - Blue-on-white.png` light) as slot [3] when logo gate = yes.
...
**For soft dimensional glass icons / polish:**
```
Cars24 brand icon system, semantic-first soft dimensional polish. The icon concept must clearly represent the slide subject: [DESCRIBE SUBJECT-SPECIFIC ICON]. Filled rounded geometric shapes with translucent pale-blue frosted-glass front layers over saturated Cars24 brand-blue base layers. Soft internal blur at overlaps. Gentle top-left highlight. Minimal details — 1–2 elements per icon. No text inside icons. Icons render directly onto the themed brand canvas in the same single composite — no transparent export, no chroma-key, no background card, no generic app tile.
...
LOGO (if requested): Cars24 logo in white on dark, vivid Brand Blue #4736FE on light (never greyed), bottom-left (or centre-bottom for centre layouts), no box/tile.
Icon to generate: [DESCRIBE ICON].
Match the translucent layered depth of the attached soft-dimensional-glass reference only where it helps. Do NOT reproduce the exact icons shown — generate contextual icons for the brief. If glass polish makes the icon generic or unclear, use Cars24 3D or flat filled style instead.
```
...
LOGO (if requested): Cars24 logo in white on dark, vivid Brand Blue #4736FE on light (never greyed or muted), bottom-left (or centre-bottom for centre layouts), no box/tile; ignore the blue tile of the logo reference.
```
...
| Hero zone | Right side or bottom-right. Character is fully contained inside the canvas, never cropped or bleeding off an edge. Scale varies: 40–60% of canvas. |
