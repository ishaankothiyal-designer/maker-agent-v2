...
LOGO (if requested): render the Cars24 logo from the attached visible logo reference at [approved logo-lock position], inside clean negative space and at the approved optical size. Relative logo path for traceability: [path]. Reproduce the rounded-square icon mark + "Cars24" wordmark faithfully in the correct theme colourway — white on dark, vivid Brand Blue #4736FE on light (never greyed or muted). For a carousel, keep official colourway, placement, and optical size fixed within every approved theme/background family. For a batch, keep exact placement and optical size fixed across every logo-bearing output, regardless of theme; only use the matching official colourway for each output background. For a standalone image, balance the correct colourway to its own layout. The logo may overlap pattern; it may overlap hero only if readable, high-contrast, cleanly fitted, and fully uncropped. No box/tile, no placeholder, no decorative effect, no cropped/cut logo, no later local overlay.
```
...
LOGO (if requested): render the Cars24 logo from the attached visible logo reference as actual visual input. Relative logo path for traceability: [path]. For a carousel, use the fixed official colourway, placement, and optical size for the approved theme/background family. For a batch, use the batch-wide fixed placement and optical size across every logo-bearing output, changing only to the official colourway required by each output background. For a standalone image, place the correct colourway in clean negative space to balance its individual layout. Reproduce the rounded-square icon mark + "Cars24" wordmark faithfully. The logo may overlap pattern and may overlap hero only if readable, high-contrast, cleanly fitted, and fully uncropped. No box/tile, no placeholder, no cropped/cut logo, no local overlay.
```
...
LOGO (if requested): reproduce the Cars24 logo from the attached visible logo reference as actual visual input — icon mark + "Cars24" wordmark — exactly, in white on dark, vivid Brand Blue #4736FE on light (never greyed or muted). Relative logo path for traceability: [path]. For a carousel, use the fixed official colourway, placement, and optical size for the approved theme/background family. For a batch, use the fixed placement and optical size across every logo-bearing output regardless of theme, changing only to the official colourway required by each output background. For a standalone image, balance the correct colourway to its layout. No cropped/cut logo or box/tile; ignore the logo reference background tile.
```
...
LOGO (if requested): Cars24 logo from the attached visible logo reference as actual visual input, in white on dark, vivid Brand Blue #4736FE on light (never greyed or muted). Relative logo path for traceability: [path]. For a carousel, use the fixed official colourway, placement, and optical size for the approved theme/background family. For a batch, use the fixed placement and optical size across every logo-bearing output regardless of theme, changing only to the official colourway required by each output background. For a standalone image, balance the correct colourway to its layout. No box/tile or cropped/cut logo; ignore the logo reference background tile.
```
