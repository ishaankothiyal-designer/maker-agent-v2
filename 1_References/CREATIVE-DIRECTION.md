...
description: Master visual creative direction for Cars24 image generation. Merges observed reference creatives (dark/light theme, pattern library) with brand guidelines into one unified system. Load this before generating any image brief or visual output.
---
...
Illustrated heroes follow a 60/30/10 colour budget with intentional brand-guideline colour richness: 60% Cars24 Brand Blue `#4736FE`; 30% supporting Deep Navy `#2B2098`, Neutral 100/off-white `#F5F5F5`, Neutral 950/near-black `#161616`, and natural warm skin tones; and at most 10% total contextual Mint Green `#63FFB1` and Orange `#EF4523`. Brand Blue remains dominant, but illustrations should not become monochrome blue blocks when the scene can benefit from approved supporting colours. Mint is a fresh/product/energy cue, including EV/charging, cashback, completion, or campaign moments. Orange is valid for cars, clothing accents, or small emphasis details. Neither mint nor orange is a default dominant colour, background, dot-pattern colour, logo colourway, or semantic status-coding system. Logo colourways remain separate and restricted to approved theme-matched white, blue, or black logo assets. Iconography remains under the separate brand-blue-monochrome rule.
...
- In a carousel, establish a theme-based logo lock before generation. In a batch, establish one batch-wide placement-and-size lock before generation: every logo-bearing output uses the exact same placement zone and optical size across all themes and backgrounds. The official colourway may change only as the background requires — white-on-brand or dark, blue-on-light, black-on-light/print — but placement and size never vary within the batch.
...
- The logo is the highest visual layer in the generated composite. It may overlap pattern and may sit over the hero, car, photo cutout, or illustration when it remains readable, high-contrast, cleanly fitted, faithful to the official lockup, and fully uncropped.
