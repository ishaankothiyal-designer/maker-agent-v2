...
The confirmed light/dark theme governs **every** format the user builds — illustrations, photographs, infographics, and USP layouts — not just card creatives, and all are produced as a single image-provider composite (no HTML/CSS creatives). Confirm it once (input collection) and apply the same output style everywhere:
...
| `Logo - Blue-on-white.png` | **Higgsfield attach asset for light / white / pale** — blue logo pre-flattened onto a white tile |
| `Logo - White.png` | Preserved raw source asset only — ⛔ but **never attach to Higgsfield** (see warning) |
...
> ⛔ **CRITICAL — `Logo - White.png` is INVISIBLE to image generators.** It is white artwork on a *transparent* background; attached as an `--image`, the transparent area flattens to a blank tile in preprocessing, so the model sees nothing and **hallucinates the wordmark from memory** — dropping the icon mark and typesetting "Cars24" in a generic font that differs every run (verified, project 012). **On dark/brand-purple, attach `Logo - White-on-blue.png`** and instruct: *"reproduce the rounded-square circular-arrow icon mark AND the 'Cars24' wordmark exactly, in white, no box/tile around it — ignore the blue background tile of the logo reference."* For light-theme generation, use `Logo - Blue-on-white.png`, not the small raw `Logo - Blue.png`.

**SVG files — preserved source assets only (HTML delivery is retired):**
...
- **Every creative with a logo** → attach/share the matching **visible** logo PNG during generation (`Logo - White-on-blue.png` on dark/brand-purple · `Logo - Blue-on-white.png` on light · `Logo - Black.png` on high-contrast/print) and instruct faithful reproduction (icon mark + wordmark) *inside* the composite. The logo is rendered by the image model during generation, never superimposed afterward by us. **Never attach the transparent `Logo - White.png`** — it is invisible (see warning above).
- The SVG files, raw `Logo - Blue.png`, and any HTML/`<img>`/base64 embedding are preserved brand/source assets and indexed in `reference-index.json`; they are not preferred generation references.
...
- If the selected provider cannot accept the correct visible logo reference, switch provider or ask the user — never approximate it and never silently add a local logo overlay
...
### Stage 1 — Inputs
Collect: write-up · for Path 3, output mode (single image, carousel, or batch create; if batch create, ask whether the user has a completed template to upload or needs `5_BATCH_EXPORT/cars24-batch-processing-template.xlsx`, then wait for completed upload) · number of slides (non-batch: single image = 1; carousel = 2–10, 2–5 recommended and above 5 confirm; batch create: use each row's `Slides per post` value) · size (1:1 · 1080×1080 / 4:5 · 1080×1350 / 1.91:1 · 1200×628 / 2:1 · 1200×600 / 16:9 · 1600×900 / Custom) · theme (single: light or dark; carousel: light, dark, or mixed)

### Stage 2 — Slide content planning
Break the write-up into N slides. For batch create, repeat this per ready row using that row's `Slides per post` value. For each slide define: title · subheading · visual message · theme (dark or light — defaults to the Stage 1 theme; for a mixed carousel, state the specific theme per slide).
...
**Generation mode: Composed creative — always, with generation-time logo reference.** Every creative is generated as a complete final image: background + pattern + subject + text layout + required logo in one shot. There is no post-process/local logo overlay.
...
| 4 — Infographic look-and-feel | All files in `1_References/1_Brand Guidelines/09_Icon-System/`; check `1_References/4_Infographic Icon References/` for prior approved icons. Choose semantic icon concepts before finish; soft dimensional glass is preferred inspiration when it improves output, with 3D/flat as clarity fallbacks. |
...
**Visible text is baked in by Higgsfield when requested.** The deliverable — whatever the style (image, illustration, pattern, abstract form, infographic) — has the requested headline and subline rendered *into* the same generated image, post-ready as-is. For Archetype 8 / No-text balanced hero, the final deliverable intentionally has no headline, subline, labels, or dormant text zone; it must still be a complete themed Cars24 composite with canvas, pattern/atmosphere, and hero/form/icon system. The subject "cutout" is not a separate exported asset and is never composited by us; it is described in the prompt and rendered directly into the composite by Higgsfield. A bare subject with no themed canvas is never the deliverable.
...
If the requested version has no visible headline/body/subtext, or visible image text is `None`, use **Archetype 8 — No-text balanced hero** by default. Rebuild the layout plan around hero, pattern, logo/stamp if any, and intentional negative space. Do not reuse a text-led layout with an empty text zone unless the brief explicitly needs room for external/manual text later. This applies across photo, illustration, infographic, and abstract. For hero-led no-text layouts, check optical centring: comparable left/right breathing room, no cutout touching or crowding one edge, and any offset hero balanced by a named counterweight.
...
- **Layer 0 — Layout & balance (from the layout plan)** — use the selected **archetype** for this slide from the Layout System (Cover/Lockup · Headline-left+Hero-right · Stacked-left+Hero-right · Text-only · Headline-dominant · Content-card overlay · Event poster · No-text balanced hero) and the matching Atlas LAYOUT reference (DT-/LT- ID in `REFERENCE-ATLAS.md`, or NT-1 for no visible text). State it explicitly, plus the **vertical anchor** (headline top / bottom / centred / full-canvas / no-text centred) and which single element dominates. Carry the balance principles into the wording: one dominant element, generous negative space, left-set text (Cover/Event centre), ~6–8% margin, hero is fully contained. If Layer 4 has no visible text, use No-text balanced hero by default: centre/enlarge the hero or expand pattern/atmosphere as needed so the whole canvas feels composed, not like a missing text layer; keep left/right breathing room comparable and avoid edge-crowded cutouts unless a counterweight is named. This decision drives the placement language in Layers 2–5.
...
**Part 1 — Reference → role mapping.** Find the references that map to each slide using the v2.0 tag index first (`1_References/reference-index.json` + `1_References/reference-tags/`; resolve indexed asset paths relative to `1_References/`), then confirm details against `REFERENCE-ATLAS.md` and `REFERENCE-SKILL-MAP.md`. Add any extra context the prompt needs (logo PNG, photography exemplar, icon-system refs, etc.). Present as a table in attach order:

| # | File (path) | Role | Attachability | Copy from reference | Ignore from reference | Why this file |
|---|---|---|---|---|---|---|
| 1 | … | LAYOUT / SUBJECT-STYLE / PHOTO-STYLE / PATTERN-TEXTURE / LOGO / PALETTE / THEME | attachable / caution / rules-only | … | … | one line |
...
**Reference-bundle preservation rule:** the approved Stage 7 reference bundle must survive into the actual generation call. If a production path cannot attach or preserve the required colour swatch, style-defining reference, or visible logo reference, do not silently degrade to prompt-only generation. Switch to a reference-capable provider path or ask before changing the logo/reference plan.
...
**Model:** default **GPT Image 2** (`gpt_image_2`). When the user wants to compare themes, generate a **dark + light variant pair**.

**Production export fidelity:** the provider must return or write a real local image file for every production output. Export means saving that exact generated bitmap into `4_exports/`, except for deterministic rename/copy/resize operations. Never run a second generation from the same prompt just to create an export file.

**External Higgsfield skills:** installed Higgsfield skills are provider utilities, not Maker source-of-truth skills. `higgsfield-generate` may be used only after the Maker Stage 1-9 approval path is complete. `higgsfield-product-photoshoot`, `higgsfield-marketplace-cards`, and `higgsfield-soul-id` remain available for explicit non-Maker tasks only.
...
3. Attach all reference images listed for that style (in the priority order shown)
4. Fire to Higgsfield. Show result. Allow revision before proceeding to the next slide.

Generate in slide order. For batch create, queue rows and generate one approved image/slide at a time in row order, then slide order within each row; do not fire multiple generations in parallel.
...
- As **each** image finishes, report the progress so far (e.g. "Slide 2 of 5 done — 3 to go") and surface that image. For production exports, surface the exact saved file from `4_exports/` using an absolute Markdown image path whenever the runtime supports it.
...
`Cars24 logo: reference the attached logo image. The logo has TWO parts — a rounded-square icon mark with a circular-arrow "C" symbol, then the "Cars24" wordmark in its specific geometric brand typeface. Place it at [location]. Reproduce BOTH the icon mark and the wordmark faithfully, in solid white, with no box or tile around it — ignore the blue background tile of the logo reference. Do not drop the icon mark, substitute a generic font, skew, stretch, recolour, or approximate.`
...
2. Score against the **Theme Fidelity Checklist** below — background hue, pattern colour/glow, headline colour, body colour, clean readable text area, fully contained hero, aspect ratio, mood. **Plus two rows for every output, any style:** (a) **copy baked in when visible text is requested** — headline + subline rendered into the image and legible; or **no visible text when Archetype 8 is requested** — no headline, subline, labels, or dormant text zone; (b) **look-and-feel matches the theme reference** — reads as the same family as the comparison `Dark theme/` / `Light theme/` creative.
...
- Missing requested copy in a text-led layout, or accidental text/labels in Archetype 8 no-text output; bare subject with no themed canvas handed off as final
...
| Layout & balance | One of the 8 Layout System archetypes, applied cleanly — one dominant element, a committed vertical anchor, generous negative space, ~6–8% margin; any text zone free of pattern/fully contained hero; no-text outputs use No-text balanced hero | Same |
...
- For premium marketing/process infographics, prefer soft dimensional glass polish when it keeps the icon readable and meaningful.
- Use Cars24 3D when object depth helps the subject read better; use Cars24 flat filled when density, small size, process clarity, or semantic precision matters more than dimensional finish.
...
**Export fidelity invariant:** the file in `4_exports/` must be the exact generated bitmap, except for deterministic file operations such as rename, copy, or an explicitly requested resize/crop. Never regenerate from the same prompt just to create an export file. If a runtime shows an image in chat but no local file can be verified, stop and ask whether to regenerate through a file-producing path; label that action as regeneration, not export.
