...
version: 2.31
changelog:
  - 2.31 — Added logo-over-hero layering and richer illustration palette guidance from Australia final batch v2 feedback. Logo-bearing outputs now treat the Cars24 logo as the highest visual layer: a hero, car, or illustration may continue behind the logo zone when the logo remains readable, high-contrast, faithful, and uncropped, and prompts must not cut, fade, or truncate the hero at the bottom merely to reserve empty logo space. Illustration prompts now use the full brand-guideline palette more intentionally while preserving brand hierarchy: Brand Blue remains dominant, but Deep Navy, Neutral 100, Neutral 950, warm skin tones, Mint Green, and Orange should be used contextually to add visual richness. Logo colourways remain restricted to approved theme-matched white, blue, or black assets. Rollback: restore v2.30 logo negative-space-only wording and stricter blue-led accent restraint.
...
9. **Export** to `4_exports/{serial}_{brief}_{DD-Mon}/vN/{item-brief}-vN-imageN.[ext]`, using the batch row's blog-title slug for `{item-brief}`. Visual QA (Stage 9) only if the user flags an output as off.
...
- **Illustration palette:** illustrated heroes use a 60/30/10 colour budget with intentional brand-guideline colour richness: 60% Cars24 Brand Blue `#4736FE`; 30% supporting Deep Navy `#2B2098`, Neutral 100/off-white `#F5F5F5`, Neutral 950/near-black `#161616`, and natural warm skin tones; and no more than 10% total contextual Mint Green `#63FFB1` and Orange `#EF4523`. Brand Blue remains dominant, but illustrations should not become monochrome blue blocks when the scene can benefit from approved supporting colours. Mint is a fresh/product/energy cue. Orange is valid for cars, clothing accents, or small emphasis details. Neither mint nor orange is a default dominant colour, background, dot-pattern colour, or semantic status-coding system. Logo colourways remain separate and restricted to approved theme-matched white, blue, or black logo assets. Icon systems remain brand-blue monochrome under their stricter rule.
...
- In carousels, establish a theme-based logo lock. Within each approved theme/background family, keep the official logo colourway, placement, and optical size exactly consistent across every logo-bearing slide. In batches, establish one batch-wide placement-and-size lock: every logo-bearing output uses the exact same placement zone and optical size across all themes/backgrounds, while the official colourway changes only as required by its background.
- The logo is the highest visual layer in the generated composite. It may overlap the pattern and may sit over the hero, car, photo cutout, or illustration when it remains readable, high-contrast, cleanly fitted, faithful to the official lockup, and fully uncropped.
- Do not cut, fade out, crop off, mask, or truncate the hero at the bottom merely to reserve empty logo space. If the approved layout places the logo over the lower hero zone, preserve full lower-body/car/hero continuity behind the logo and describe the logo as an overlay layer above the composition. The hero behind the logo may be visually subordinate, but it should continue naturally rather than ending in a hard artificial crop.
...
**Level 3 — Files inside the version folder:** `generation-manifest.md` + `{item-brief}-vN-image1`, `{item-brief}-vN-image2`, `{item-brief}-vN-image3` …
...
- Name each output `{item-brief}-vN-imageN.png`, where `{item-brief}` is the kebab-case blog-title slug for a batch row and the short project-brief slug for a non-batch creative. `vN` must match the enclosing version folder so files remain identifiable when separated from their folder.
- One file per slide. Slide 1 → `{item-brief}-vN-image1.png`, slide 2 → `{item-brief}-vN-image2.png`, etc.
- A single (non-carousel) image is `{item-brief}-vN-image1`.
...
Export is not a separate optional cleanup step. Immediately after every successful generation run, copy or rename the exact produced bitmap(s) into the target `4_exports/{serial}_{brief}_{DD-Mon}/vN/` folder, then write or update `generation-manifest.md`, then present those exported files as the canonical result. Do not leave successful outputs only in temporary provider folders, chat previews, or Codex generated-image caches.

- **Codex `image_gen`:** if the exact generated bitmap is locally accessible, export that same file immediately into `4_exports/`. Do not leave export as a deferred manual follow-up step.
- **If exact-file export is blocked:** stop and ask before any regeneration, provider switch, fallback, or substitute file-writing pass.
- **Export fidelity:** export means copying, renaming, or an explicitly requested resize/crop of the same generated bitmap. Export never means regenerating the image just to populate `4_exports/`.
...
- **60% Brand Blue** `#4736FE` — dominant: car interiors, large clothing shapes, environments, and main scene elements
- **30% Supporting tones** — Deep Navy `#2B2098` (depth, shadow, darker shapes) · Off-white/Neutral 100 `#F5F5F5` (highlights and light objects) · Neutral 950 `#161616` / near-black navy (hair, tyres, deep contrast) · natural warm skin tones
- **10% total controlled accents** — Orange `#EF4523` and Mint Green `#63FFB1`, used intentionally to make the illustration feel more alive. Mint can signal freshness, product energy, EV/charging, cashback, or a campaign cue. Orange can appear on cars, clothing accents, or small emphasis details when contextually useful. Do not use both by default, and keep the combined accent area within the 10% allocation.
...
**Accent rule:** Use approved brand accents to enrich illustration context, not to recolour the brand system. Orange may appear on cars, clothing accents, or small emphasis details. Mint may appear as a fresh/product/energy cue, including EV/charging, cashback/rebate, completion, or campaign moments. Neither is dominant, a background, a dot pattern, a logo colourway, or a generic semantic status-coding system; together they remain within the 10% accent allocation. Brand Blue is still the correct dominant.
...
```
Modern sleek flat editorial illustration, [SCENE], South Asian characters with warm caramel skin tones, electric brand blue dominant (60%), intentional supporting tones from deep navy, off-white, near-black, and natural skin colours (30%), and contextual fresh mint green and/or vivid orange accents together no more than 10%. Use the approved brand palette to add life and specificity without making the image orange- or mint-dominant. Clean flat colour shapes with minimal shading, no photorealistic textures, deep navy-black hair, clean crisp silhouette edges, aspirational and confident mood. Render the subject as a clean-edged hero directly inside the themed brand canvas generated in the same composite, never in a rectangle, scene panel, or gradient box. Framing: [contained OR intentional editorial edge crop]; keep faces, focal interaction, action-carrying hands, and meaning-carrying product detail clear, while allowing a supporting car, shoulder, lower body, car body, or restrained contextual environment to continue behind a selected logo overlay when it improves composition. Do not cut, fade, or truncate the bottom of the hero merely to reserve logo space. The headline, optional support line, and optional Cars24 logo are other layers of the same composite, not part of the subject art; when selected, the logo is the highest overlay layer rendered from the theme-matched logo reference during generation, using only approved white/blue/black logo colourways. Style: premium modern sleek flat editorial illustration.
```
...
> - Add orange when the scene benefits from a contextual car, clothing, or small emphasis detail; keep it clearly secondary.
> - Add mint when the scene benefits from a fresh/product/energy cue such as EV charging, cashback, completion, or a campaign signal; keep it clearly secondary.
