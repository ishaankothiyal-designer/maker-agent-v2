...
2. Route to the correct path based on the user's choice:
   ...
   **Path 2 — Write-up + image:**
   ...
   Step B: choose single image or carousel → if carousel, ask slide count with 3 as suggested default → image format → optional visual direction → generate image brief → trigger Higgsfield.
...
- Infographic look-and-feel → `09_Icon-System/` all 3 files; check `4_Infographic Icon References/` for prior icons. Default sub-styles: **3D** (premium marketing/feature callouts) or **flat filled** (dense/process/UI flows). **Glass** is an explicit sub-style choice, not the silent default — only use when the user selects it. All icon sub-styles use **brand-blue `#4736FE` monochrome only** (NO green, NO cyan, NO teal, NO mint, NO red, NO orange, NO yellow)
...
**Stage 7 — ⛔ Assembly & Approval gate (MANDATORY before any Higgsfield call)**: no `higgsfield generate` until all are assembled and shown to the user — (1) a **reference → role mapping table** with file, role, attachability, copy-from, ignore-from, and Higgsfield `--image` transport status; (2) the **v2.5 style-purity audit plus v2.6 layout plan plus v2.8 photo-cutout guard when style is photo/image-led**; (3) the **fully assembled prompt**, including relative logo path for traceability when logo=yes; (4) requested/provider ratio mapping and a **real Higgsfield credit estimate** obtained after `higgsfield account status`; and (5) an explicit approval ask. Gate cannot pass if a slide blends primary styles without explicit user approval, if a carousel/batch repeats the same layout without an explicit reason, if a photo/image-led prompt lacks cutout + white outline or uses a rectangular frame/full-scene photo without explicit reason, or if a required reference cannot reach Higgsfield. Batch jobs define 3–5 approved visual territories that include both style and layout, then assign every row to one territory. Load `HIGGSFIELD-CONTEXT-PACKAGE.md` AND `CREATIVE-DIRECTION.md` first — if either is unloaded the prompt is thin, do not fire. Generate only on yes.

**Stage 8 — Higgsfield** (precondition: Stage 7 gate approved): run `higgsfield account status`; if it fails, stop and ask the user to run `higgsfield auth login`. With `1_References/HIGGSFIELD-CONTEXT-PACKAGE.md` already loaded, find the Package matching the visual style → copy the Prompt Context Block → prepend it to the prompt → map unsupported ratios visibly → attach the approved reference files in map order. Fire each slide prompt with GPT Image 2 (`gpt_image_2`) unless the user explicitly requested another supported model, with the correct theme logo PNG attached as visual input; generate in order. If a provider/tool cannot attach the visible logo PNG, do not generate from text-only logo prompting; stop and ask the user to share/upload the logo so it can be used as visual input, or move to a reference-capable provider/workflow. **Report progress to the user while generating** — start, each image as it completes, and batch complete; show each result.

**Stage 9 — Visual QA (on-demand only)**: not a mandatory gate — skip unless the user flags an output as wrong. If they do, open the output next to the matching theme reference creative and score it against the Theme Fidelity Checklist (`master-rules.md §4`): background hue, pattern colour/glow/opacity (light dots ≈20–25%), Arapey-led serif headline dominance in both themes, text colour mapping, layout & balance, clean readable text area, fully contained hero, ratio, mood, copy baked in, same-family look-and-feel, generated logo fidelity if logo=yes, **icon fill colour** (brand-blue monochrome only — fail if any icon contains green/cyan/teal/orange/red), **icon style fidelity** (must match the selected sub-style: 3D/flat/glass), and **light-theme element saturation** (headlines, logo, and icons must be vivid `#4736FE`, not desaturated/greyed). The same checklist binds photos and illustrations equally (never tint a photo to fake a match). Any fail → regenerate naming the exact deviation, then re-check.
